  ...
  return (
    <ol className="flex flex-col">
      {PIPELINE_STAGES.map((stage, idx) => {
        ...
        const dotClass =
          status === "completed"
            ? "border-av-green bg-av-green text-white"
            : status === "skipped"
        ...
        return (
          <li key={stage.id} className="relative flex gap-4 pb-8 last:pb-0">
            ...
            <span
              className={`z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 text-xs font-bold ${dotClass}`}
            >
